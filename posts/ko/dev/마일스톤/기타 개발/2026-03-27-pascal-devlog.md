...
프로그램 이름이 Pascal인 이유는 최초로 PDF 압축 기능을 목표로 개발을 시작했기 때문입니다. 정작 압축 기능은 소집해제를 목전에 앞두고 있어 수지타산이 맞지 않아 구현하지 않았지만, PDF 파일 조작이라는 맥락은 병합과 분할 두 가지로 먼저 구현되었습니다.
...
기관을 특정할 수 있는 정보를 공개하는 것은 병무청이 권장하지 않는 행동이고, 그 입장을 존중해 어떤 상황에서 어떻게 활용되었는지 정확히 묘사하기는 어렵지만 간략히는 기술할 수 있을 것 같습니다. 먼저 이 프로그램을 내가 소유하고 있다는 감각이 심리적으로 큰 도움이 되었고, 다음으로 이전 파이썬 스크립트를 돌리기 전 `config.yaml` 설정값을 매번 수정하는 등 날것 그대로의 업무 형태가 몇 개 버튼 클릭으로 보기좋게 간소화되는 이점이 있었습니다.
...
윈도우 프로그램을 개발에 있어서 사전 지식, 경험이 없는 상태였고 프레임워크를 선택하는 단계에서부터 방향을 쉽게 잡지 못했습니다. 처음에는 프론트엔드 개발 경험을 재사용해보고자 [Electron.NET](https://github.com/ElectronNET/Electron.NET)을 처음 찾아봤다가, 당사자들부터가 "Wait - you host a .NET Core app inside Electron? Why?"라고 되묻고 있어 정석적인 방향과 많이 동떨어져있다는 인상을 받았습니다. 그리고 때마침 [Fluent 2](https://fluent2.microsoft.design/)라는 마이크로소프트 디자인 문법을 알게 되면서 [ModernWPF](https://github.com/Kinnara/ModernWpf) 라이브러리를 사용하는 WPF 프로젝트로 전환했고, 개발 도중 레거시 환경이 2025년 기준으로는 낙후되어있다고 느껴져 최신 프레임워크 WinUI 3로 개발환경을 옮겼습니다.
...
하지만 막상 깃허브 외에는 기대한 만큼의 호응이 없었고, 사실 조금 머쓱했습니다. 동료가 게을러서는 아니었고, WinUI 3라는 새 환경도 벅찬데 노션, 피그마라는 도구가 왜 필요한지 설득력이 부족했던 것 같습니다. 저는 자원 관리 체계가 개발을 더 멀리 지속시킬 수 있으므로 언제나 필요하다고 생각했던 반면, 동료분께서는 개발과 크게 상관없는 비용이라면 필요에 따라 내칠 수 있다고 보셨던 것 같습니다. 그리고 차차, 여기서는 그 생각이 맞다는 생각이 들었습니다. 객체지향의 효용성과 마찬가지로 좋은 도구와 좋은 패턴에도 그것이 정당화되는 규모가 있다는 것을 느꼈습니다. 좋은게 좋은 것이라며 도구를 관성적으로 맹신했던 것 같습니다.
...
프로그램 개발 도중 이 일화가 생각났습니다. 예술작품을 만들 때 들기 마련의, 정신적 감응, 자아실현의 절박함, 이런 것들이 프로그램 개발에 필요한 것은 아니므로 엄밀히는 저와 상황이 다릅니다. 하지만 소모적인 업무를 매일 처리해야 하는 환경이 창작과 작업 추진 의지를 얼마나 지치게 만드는지에 대해서는 공감되는 부분이 있었습니다.

실제로 이 프로젝트는 그런 이유에서 진척이 힘들었습니다. 어떤 UI 컨트롤을 어떻게 사용할 수 있을지 상상하고 있을 때 업무 관련해서 불려나가고, 20분 후 자리에 돌아와서 맥락을 다시 복구하다보면 자리 전화벨이 울렸습니다. 창의력이 필요한 순간 뿐만 아니라, M-V-VM간 의존성 흐름을 검토하는 등의 단순 디버깅 작업도 잦은 방해로 집중이 어려웠습니다. 부당한 상황이었다고 투덜거리고 싶은건 아닙니다. 다만 환경적 여유가 뒷받침되지 않을 때 일 능률이 어디까지 떨어질 수 있는지를 이번에 처음으로 확인했습니다.
