...
개인적으로 프로그래밍 경험이 깔끔하지는 않아 아두이노를 별로 좋아하지는 않습니다. 그런데 최근에 아두이노를 사용할 기회가 한 번 더 있었습니다.

달갑진 않았지만, 적당히 시간 때우고 가기보다는 이왕 하는 거 새로 뭐라도 배워가자는 생각이 들어 객체지향 설계에 도전했습니다. 아두이노는 C++을 기반으로 동작하는데, 파이썬이나 C#과는 다른 모습이 있어 조금 생소했지만 그래도 잘 동작하는 모습까지 확인하고 나니 뿌듯합니다. 혹시라도 나중에 C++을 사용할 일이 있을 때 참고할 수 있으면 좋겠습니다.
...
간단한 금액을 추가하는 기능의 코드를 낱개로 분할하여 구현한 예시입니다. 클래스의 선언부인 `MainFunctions.h`에서 메서드를 선언하고, `MainFunctions.cpp`에서는 선언된 메서드를 정의하는 역할을 맡고, `Arduino_OOP.ino`에서는 정의된 메서드를 호출하는 역할을 맡도록 작성했습니다.
