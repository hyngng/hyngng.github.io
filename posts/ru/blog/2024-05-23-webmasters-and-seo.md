...
`content` проходит через пользовательский Ruby-плагин `content_filter`, который удаляет ненужную для description информацию: заголовки, дату публикации, автора и вступление «Введение». Использован тот факт, что весь текст статьи находится в теге `<div class="content"></div>`. Хотя у меня [был опыт написания похожего кода](https://hyngng.github.io/ru/blog/blog-content-remove/), я всё ещё не очень熟悉, поэтому за советом обратился к GPT.
