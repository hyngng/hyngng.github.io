...
[В предыдущем проекте](https://hyngng.github.io/ru/dev/armonia-second-devlog/) я использовал эту функцию, как показано выше. Когда игрок касается определённого объекта, этот объект активируется и регистрируется в `ActivatedObject` у `MainManager`. Всё работало удовлетворительно.
