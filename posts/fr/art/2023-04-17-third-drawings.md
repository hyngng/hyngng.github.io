---
image:
    path: /2023-04-17-third-drawings/preview-image.webp
    lqip: data:image/webp;base64,UklGRkYAAABXRUJQVlA4IDoAAACwAQCdASoQAAgAAgA0JaQAAtz+deMAAP7+fCmBZZn3+/ZORq9T+vedgOywrM/PuK61PMAYYBLZKgAA
    alt: 썸네일용

title: "Dessin numérique #3 : Changeons de format"
authors: ["art"]

categories: [그림]
tags: [그림, 드로잉, 기지개, 어안렌즈, 폰카]
start_with_ads: false

toc: false
 
date: 2023-04-17 15:24:00 +0900
last_modified_at: 2023-04-26 20:55:00 +0900
---

![230426-거꾸로매달림](/drawing/230426-거꾸로매달림.webp){: .rounded-10 .light .border }
![230426-거꾸로매달림](/drawing/230426-거꾸로매달림.webp){: .rounded-10 .dark }
*Suspendu à l'envers*

![230329-스트레칭](/drawing/230329-스트레칭.webp){: .rounded-10 .light .border }
![230329-스트레칭](/drawing/230329-스트레칭.webp){: .rounded-10 .dark }
*De bonne humeur quand même*

![230427-빤히](/drawing/230427-빤히.webp){: .rounded-10 .light .border }
![230427-빤히](/drawing/230427-빤히.webp){: .rounded-10 .dark }
*Regard fixe*

![200000-기쎈여자옆모습](/drawing/200000-기쎈여자옆모습.webp){: .rounded-10 }
*Plutôt une forte impression*

![230426-어안렌즈](/drawing/230426-어안렌즈.webp){: .rounded-10 }
*Vu d'en bas avec un fisheye*

![230330-핸드폰사진](/drawing/230330-핸드폰사진.webp){: .rounded-10 }
*Imiter une mise en scène de cadre*

J'ai envie de dessiner quelque chose de nouveau, alors je tords même les mêmes sujets à ma façon, et c'est vraiment plus amusant à dessiner. Tout le monde pense pareil, j'imagine, mais moi aussi, si je dois dessiner, je préfère l'original au banal.

Récemment, en discutant avec une connaissance des espaces d'erreur dans la perspective, j'ai pensé que la perspective honnête est conceptuellement similaire à la distorsion d'image en photographie. Plus c'est près, plus c'est grand ; plus c'est loin, plus c'est petit — si on doit décrire ainsi, alors la forme à travers un objectif fisheye ne serait-elle pas l'idéal ? C'est sur cette conclusion que la conversation s'est terminée.
