...
- `author`: `src/settings/authors.settings.ts`의 author id로 정규화, 기본값 `dev`.
...
현재 등록된 author id는 `blog`, `dev`, `art`, `essay` 4개임. `src/settings/authors.settings.ts`의 `getAuthor()` 함수는 미등록 author id가 들어오면 throw하여 빌드를 실패시킴.
...
- **`BaseLayout.astro`**: `<Head>`에 자식으로 `<slot name="head">`를 전달. 포스트가 아닌 페이지(홈, 작가)에서는 이 슬롯의 기본값으로 전역 OG 태그(`og:title`, `og:description`, `og:type="website"`, `og:url`, `og:site_name`, `twitter:card`)가 렌더링됨.
...
- `og:title`, `og:description`, `og:type="website"`, `og:url`, `og:site_name`, `twitter:card`

**포스트 오버라이드 (PostLayout 슬롯):**
- `og:title=post.data.title`, `og:description=post.data.description`, `og:type="article"`, `og:url=Astro.url.href`, `og:site_name=SITE.title`, `twitter:card=summary`
...
- `LoadMoreCard` 호버 필터: 기본 `brightness(0.9) opacity(0.9)` 상태에서 호버 시 `brightness(1) opacity(1)`로 밝기/투명도가 복원된다.
