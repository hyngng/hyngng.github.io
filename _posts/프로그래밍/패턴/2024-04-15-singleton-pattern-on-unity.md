...
그럼에도 원래는 블로그 분위기가 너무 딱딱해질까봐 기술적 개념을 다루는 포스트는 일부러 자세히 작성하지 않으려 했는데, 최근 블로그의 어원이 Web + Log라는 것을 알게 되고 공부한 것을 정리한 글고 나름 괜찮겠다는 생각이 들었습니다. 그래서 앞으로 따로 기록이 필요한 것들은 아예 한 번씩 심도있게 짚고 넘어갈 겸 블로그에 차근차근 하나씩 정리해두려고 합니다.
...
단, 싱글톤 패턴은 남용이 쉽다는 구조상 논란이 있는 프로그래밍 패턴입니다. 싱글톤이 너무 편리한 나머지 스크립트가 너무 많은 역할이나 데이터를 담당하게 되면 싱글톤 인스턴스와 다른 클래스간 결합이 강해지며, 지저분해진 코드는 인스턴스의 변경 주체나 시점을 알기 어렵게 만든다는 겁니다.
