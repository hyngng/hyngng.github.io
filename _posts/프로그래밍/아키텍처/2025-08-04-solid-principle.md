...
date: 2026-03-12 15:30:00 +0900
last_modified_at: 2025-03-12 15:30:00 +0900
...
![](/2026-04-01-oop-and-srp-dip-princlple/something.png)
*더닝 크루거 효과*

저는 좋은 프로그램 설계를 모릅니다. 그런데 이렇게 모른다고 할 수 있는건 아직 끝까지 파보지 않았다는 것이고,

유명한 더닝 크루거 효과로 비유하자면

## **객체지향에 대한 단평**

객체지향이 고안된 배경, 객체지향이 잘 하는 것들은 잘 알려져 있다. 종래의 절차지향 패러다임이 지닌 한계의 타파하기 위해서, 세부적으로는 추상적 사고를 보존하고 복잡한 비즈니스 로직을 사람이 이해하기 편한 방식으로 자연스럽게 설계하기 위해서. 하지만 왜 이 방법론이 큰 호응을 얻었는가는 덜 다뤄지는 모양새다. 그리고 이와 관련해 내가 찾은 관점이 두 개 있다. 사업과 철학이다.

하나. 무언가를 책임진 사업가, 야망가는 불확실성을 싫어하기 마련이다. 국경을 접한 북방 제국이 한반도계 국가를 끊임없이 침략한 이유 중 하나는 대륙 정벌이라는 사업을 달성하기 이전에 후방을 안정시키기 위해서였다. 로마가 카르타고를 치기 전에 이탈리아 반도를 먼저 통일한 사례, 독일은 프랑스 침공 이전에 소련과 불가침조약을 체결한 사례도 행위 동기는 크게 다르지 않다. 사업이 클 수록, 임하는 마음이 진지할 수록 블랙스완 차단은 필수적이다.

둘. 현대물리학자가 대통일 이론을 열망하는 것, 언어학자가 세계의 모든 언어 사이에 보편문법이 있다고 가정하는 것, 경제학자가 수백만 명의 행동을 수요와 공급이라는 하나의 그래프로 설명하려 하는 것 등은 모두 난잡함을 통제해내려는 의도에서다. 왜 그런 의도를 갖게 될까, 그 이면에 인지비용을 낮추려는 실용적인 이유도 있겠지만 나는 실존주의적인 설명을 더 좋아한다. 예컨대 "인간은 불확실성과 모호함을 견디지 못하고 명료한 인식관을 갈구하기 마련이다"라는 것.

확신은 없다. 하지만 직관으로는 객체지향은 이 두 원리, 위험 관리 기질과 인간 본성이 프로그래밍에서 작용한 결과로 보인다. 이 패러다임은 전통적 절차지향 코드를 나누어 적재적소에 정리하고, 그 구조와 이름에 더 많은 단서를 남긴다. 결과적으로 불확실성이 줄고 복잡도가 통제되며 개발자는 안정감을 얻는다.

### **객체지향의 의의**

객체지향은 사람이 이해하기 쉬운 방식으로 프로그램을 개발하기 위한 것이지 기계의 입장에서 고안된 것이 아니다. 이 관점에서 객체지향의 반대편에 서 있는 패러다임 ECS<sup>Entity Component System</sup>를 비교해보기 좋다.
...
- 모듈화식 프로그래밍을 할 수 있다고 들어서 멋져보여서 공부해놨더니 실제로는 '서로 연관시키기', '커플링'을 통해 구현하게 됨. 실제로 이게 OOP의 핵심은 맞음. 기본적으로 결합시키는 것을 전제로 최대한 결합을 낮추는 역방향 사고를 하는 것임.

### **작명하기 좋은 환경의 이점**

- 이름짓기는 사고의 구체화에 매우 좋음.

사람이 이해하기 쉬운 방식이라는 것은 단순히 객체에 동작을 할당하는 것 뿐만은 아닙니다. 제 생각에 객체지향의 숨은 장점은 구조에 대한 단서가 구조적으로 더 많이 남는다는 겁니다.

예를 들어 `Chief.Cook()`과 같은 코드는 그 자체로 주석을 대체합니다. 잘 지어진 이름에서는 클래스도 메서드도 역할이 이름에 명시되기 때문입니다.
...
이름짓기는 철학적으로 정말 중요한 행위입니다. 그 자체로 환경을 인식하기 위한 행위입니다. 예전부터 <sup>無名天地之始, 有名萬物之母</sup>

암묵지<sup>tacit knowledge</sup>라는 개념이 있습니다.
...
비슷하게 글쓰기에는 한 개의 문장에서는 한 가지의 주제만을 다루라는 일문일사<sup>一文一事</sup> 원칙이 있다. 의미의 명료함을 달성하는 것, 단일 책임 원칙도 이 맥락을 같이 한다.

하지만 이건 엄밀히는 부수적인 효과다. SRP의 핵심은 정확한 코드 분류가 아니다. 물론 그런 효과를 달성해야 하는 면은 있지만, SRP가 다루는 것은 더 넓은 부분이다. SOLID 원칙은 기본적으로 살아 숨쉬는 프로그램을 상상하며 정립된 개념이라는 점을 인식해야 한다. 상황은 바뀌기 마련이고, 프로그램도 변화된 환경에 적응해야 한다.

이 관점에서 단일 책임 원칙의 의의는 변경의 예측가능성을 달성하는 것에 있습니다.

해석가능성을 한 개로 통제하라는 의미도 되겠다.




이어서 다룰 SRP, DIP 원칙은 SOLID 원칙에서 각각 S, D에 해당하는 부분입니다.

변화를 통제하기 위한 겁니다.

그리고 이 유사함은 제 생각에 우연이 아닐 것 같습니다.

불확실성.

변경의 파급 범위를 예측 가능하게 하는 것
...
DIP 원칙을 디루면서 조직, 특히 회사가 스스로를 관리하는 방식이 계속 떠올랐다. 부서와 그 구성인원간 업무분장이 있는 전형적인 회사 구조에서 DIP에 따르면 회사는 사회적으로 먼저 합의된 개념, 업무분장에 대해서만 의존해야지 그 업무를 수행하는 직원의 개인기나 기량에 의존해서는 안 된다.

이건 맞는 말이다. 회사의 나날은 고정된 것처럼 보이고 어제도 오늘도 내일도 그 직원이 그 자리에서 같은 업무를 수행할 것처럼 느껴지기 마련이다. 하지만 현실적으로 정기적 발령이나 급격한 인사이동, 퇴사, 극단적으로는 버스 팩터<sup>Bus Factor</sup>에서의 끔찍한 가정처럼 업무 담당자가 사망하는 등의 이유로 직원이 교체되는 일은 일어날 수밖에 없다. 만약 조직이 업무분장을 넘어 개개인의 역량에까지 의존하고 있었다면 어떠한 이유로든 그 사람이 부재하게 되었을 때 혼선이 빚어질 것이다.

이 관점에 따르면 한 사람에게 합의된 범주를 넘어선 업무가 위임되는 상황을 막아야 하는 이유는 그것이 한 사람의 선의를 이용하는 비도덕적인 행위여서이기 이전에 조직이 와해되는 위험을 야기할 수 있기 때문이다. 예를 들어 조직에 요구되는 업무량이 늘어난 경우, 그때그때 개개인에게 업무를 위탁하는 대신 업무분장 등 책임이 분배되는 구조를 근본적으로 개선하는 식으로 해결되어야 한다. 재밌는 것은 이 감각은 기술적으로 번역하면 원문이 된다. '인터페이스를 상속받은 구현체가 아닌, 인터페이스 그 자체에 의존하라'

직원이 회사에 맞춰 일하는 것(직원이 업무 분장에 의존)뿐만 아니라, 회사 역시 직원의 개인기에 기대는 대신 객관적인 업무 분장표에 의존하게 되므로 의존성의 방향이 '역전'된다

## ****

OOP도 결국엔 방법론이고 경직된 전통으로 남지 않기를.

특히 최근 바이브 코딩이 새 트렌드로 떠오르게 되면서, 앞으로도 효과적인 방법론으로 남을 수 있을지는 의심되는 면이 있습니다.
