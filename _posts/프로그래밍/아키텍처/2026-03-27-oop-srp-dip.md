...
그래서 의심이 든다. 이미 성능에 있어서는 DOD라는 훌륭한 접근이 있고, 상태 관리의 복잡성에 있어서는 함수형 패러다임이라는 대안이 있다. 물론 얼마든지 이 이상 장수할 수도 있다. 하지만 최근 바이브 코딩부터 하네스 엔지니어링까지 최근 트렌드는 코드 설계를 사람이 직접 들여다보지 않는 방향으로 발전하고 있고, 앞으로도 코드를 객체로 작성해야만 하는지에 의문부호가 붙는다. 어떤 패러다임이든 오래오래 경직된 전통으로 남는 일은 없기를 바란다.
