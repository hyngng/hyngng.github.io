...
기관을 특정할 수 있는 정보를 공개하는 것은 병무청이 권장하지 않는 행동이고, 그 입장을 존중해 정확히 어떻게 활용되었는지 묘사하기는 어렵지만 간략히만 기술할 수 있을 것 같습니다. 먼저 이 프로그램을 내가 소유하고 있다는 감각이 심리적으로 큰 도움이 되었고, 다음으로 이전 파이썬 스크립트를 돌리기 전 `config.yaml` 설정값을 매번 수정하는 등 날것 그대로의 업무 형태가 보기좋게 간소화되는 이점이 있었습니다.
