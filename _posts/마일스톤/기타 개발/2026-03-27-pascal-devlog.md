...
last_modified_at: 2026-05-04 13:13:00 +0900
...
프로그램 이름이 Pascal인 이유는 최초로 PDF 압축 기능을 목표로 개발을 시작했기 때문입니다. 압축 기능은 수지타산이 맞지 않아 구현되지 않았지만 PDF 파일 조작이라는 맥락은 살아남아 병합과 분할 두 가지 기능 구현으로 이어졌습니다.
...
하지만 막상 깃허브 외에는 기대한 만큼의 호응이 없었고 조금 머쓱한 상황이 있었습니다. 동료가 게을러서는 아니었고, WinUI 3라는 새 환경도 벅찬데 노션, 피그마라는 도구가 왜 필요한지 설득력이 잘 없었던 것 같습니다. 저는 자원 관리 체계가 개발을 더 멀리 지속시킬 수 있으므로 언제나 필요하다고 생각했던 반면, 동료분께서는 개발과 크게 상관없는 비용이라면 필요에 따라 내칠 수 있다고 보셨던 것 같습니다. 그리고, 차차 여기서는 그 관점이 맞다는 생각이 들었습니다. 객체지향의 효용성과 마찬가지로 좋은 도구와 좋은 패턴에도 그것이 정당화되는 규모가 있다는 것을 느꼈습니다. 좋은게 좋은 것이라며 도구를 관성적으로 맹신했던 것 같습니다.
