...
이 경험은 사후적으로 짚고 싶은 것이 하나 있습니다. 프로젝트를 열기 전에 찾아본 바에 의하면 이크로소프트는 지난 20여 년간 WinForms를 시작으로 WPF, UWP, WinUI 3, 그리고 최근의 MAUI에 이르기까지 구 플랫폼과 완벽히 호환되지 않는 새 프레임워크를 잔뜩 쏟아냈고, 개발자 입장에서 무엇을 표준으로 잡고 개발해야 하는지 정답이 사실상 없는 만들었습니다. 윈도우 프로그램 개발 프레임워크는 게임이나 모바일, 웹 개발과 달리 선택 기준이 모호한 것이 사실이고, 프로젝트를 막 시작하던 당시에는 잘 몰랐지만 제 삽질은 조금은 당연한 것이었습니다.
