...
이 경험에서 사후적으로 짚고 넘어갈 부분이 하나 있습니다. 찾아본 바에 의하면 마이크로소프트는 지난 20여 년간 WinForms를 시작으로 WPF, UWP, WinUI 3, 그리고 최근의 MAUI에 이르기까지 구 플랫폼과 완벽히 호환되지 않는 새 프레임워크를 잔뜩 쏟아냈고, 개발자 입장에서 무엇을 표준으로 잡고 개발해야 하는지 정답이 사실상 없게 만들었습니다. 윈도우 프로그램 개발 프레임워크는 게임이나 모바일, 웹 개발과 달리 선택 기준이 모호한 것이 사실이고, 제 삽질은 2026년 윈도우 프로그램 개발을 처음 시도하는 입장에서 약간은 통과의례였던 것 같습니다.
...
WPF, WinUI 3의 첫인상이 좋았던 부분은 개발 경험이 프론트엔드 웹 개발과 매우 유사하다는 점입니다. 좀 재미있었습니다. XAML로 UI 구성 요소와 속성을 정의하고 C#으로 세부 로직을 작성하는 과정이 HTML과 자바스크립트가 동작하는 방식과 같고, 스타일 정의 또한 CSS, SCSS를 다뤄본 적 있다면 어렵지 않아 UI 프레임워크는 금방 적응할 수 있었습니다.

그러나 WinUI 3 생태계는 마이크로소프트의 지속적인 지원에도 불구하고 여전히 빈약한 편입니다. 예컨대 [WinUI-3-Apps-List 프로젝트](https://github.com/DesignLipsx/WinUI-3-Apps-List?tab=readme-ov-file)를 둘러보면 양 자체는 어느 정도 있지만 질이 높지 않습니다. 특히 오픈소스로 코드가 공개된 경우는 더더욱 드물기 때문에 이 프레임워크를 다른 사람들이 실제로 어떻게 다루고 있는지 알기 어려웠습니다. 그래서 WinUI 3 프로젝트를 어떻게 이끌어야 하는지에 대한 고민은 [마이크로소프트에서 제공하는 공식 문서](https://learn.microsoft.com/ko-kr/windows/apps/winui/winui3)와 씨름하며 연역적으로 풀어야 했는데, 한국어 번역본의 완성도가 낮아 영문 문서가 사실상 유일한 선택지였고 LLM도 이 영역에서는 큰 도움이 되지 않았습니다. 초기 학습 비용이 있었던 부분입니다.
