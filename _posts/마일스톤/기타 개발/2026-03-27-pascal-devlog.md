...
last_modified_at: 2026-05-14 09:15:00 +0900
...
VSCode의 미니멀리즘한 디자인을 좋아하는 덕에 먼 친척 Visual Studio는 사용할 줄은 알지만 코드 실행 방법만 아는 정도였습니다. 더구나 WinUI 3라는 생소한 프레임워크로 시도해본 적은 없었습니다. 솔루션, NuGet, 디자이너 같은 용어나 메뉴 바에서 제공되는 기능에 익숙해지는 것 자체가 첫 번째 병목이었고, 기능 구현 이전에 개발 환경의 구조를 이해하는 데 생각보다 많은 시간을 써야 했습니다.
...
실제로 이 프로젝트는 그런 이유에서 진척이 힘들었습니다. 어떤 UI 컨트롤을 어떻게 사용할 수 있을지 상상하고 있을 때 업무 관련해서 불려나가고, 20분 후 자리에 돌아와서 맥락을 다시 복구하다보면 자리 전화벨이 울렸습니다. 창의력이 필요한 순간 뿐만 아니라, M-V-VM간 의존성 흐름을 검토하는 등의 단순 디버깅 작업도 잦은 방해로 집중이 어려웠습니다. 부당한 상황이었다고 투덜거리고 싶은건 아닙니다. 다만 환경적 여유가 뒷받침되지 않을 때 일 능률이 어디까지 떨어질 수 있는지를 이번에 확인할 수 있었습니다.
...
라이브러리 중 DevWinUI는 [ghost1372](https://github.com/ghost1372)라는 이름으로 활동하는 이란 개발자 Mahdi Hosseini에게 전적으로 관리됩니다. 공개된 정보로 이분은 이란 Qeydar 시에서 교사로 일하며 거주하고 있는 것으로 보입니다. 그리고 정말 말도 안 되는 이야기인데, 2025년 12월 28일 이란 전역에서 대규모 시위가 발생함에 따라 이란 당국이 이를 강경 진압하기 시작했습니다.
