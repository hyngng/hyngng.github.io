...
다만 `jekyll-polyglot`은 기본적으로 유연하고 편리하다는 느낌보단 거추장스러운 느낌이 강합니다. 이럴거면 영어 전용 페이지를 별도로 개설하는게 낫지 않을까 싶기도 했네요. 그래도 페이지 구성 파일들을 공유 가능하고, 동일 웹 주소로 웹마스터도구, 애드센스, 애널리틱스 등을 별도로 관리해야 하므로 자체 다국어 지원 기능을 만들만한 것 같기는 합니다.