...
구현 과정이 조금 골치아팠습니다. description을 포함한 메타 태그는 `jekyll-seo-tag` 플러그인을 통해 먼저 일괄 생성되므로, 생성된 `seo_tag` 중 description을 오버라이딩 하는 식으로 구현했습니다. 구현 도중 `head.html`{: .filepath }을 포함한 `_includes`{: .filepath } 폴더의 파일은 페이지 콘텐츠에 접근할 수 없는 문제가 있었으나 `_layouts/default.html`{: .filepath }에서 `content`를 조달해 사용하는 식으로 보완했습니다.
